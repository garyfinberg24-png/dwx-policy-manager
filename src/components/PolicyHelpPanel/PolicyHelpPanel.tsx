...
export interface IPolicyHelpPanelProps {
  isOpen: boolean;
  onDismiss: () => void;
}

interface IPolicyHelpPanelState {
  currentTab: HelpTab;
  searchQuery: string;
...
export class PolicyHelpPanel extends React.Component<IPolicyHelpPanelProps, IPolicyHelpPanelState> {
  constructor(props: IPolicyHelpPanelProps) {
    super(props);
    this.state = {
      currentTab: 'home',
      searchQuery: '',
    ...
    return (
      <div>
        ...
        <div className={styles.articleContent}>
          <h2 style={{ marginTop: 0, marginBottom: '8px' }}>{selectedArticle.title}</h2>
          <p style={{ color: '#605e5c', marginBottom: '20px', fontSize: '13px' }}>Category: {selectedArticle.category}</p>
  ...
  public render(): React.ReactElement<IPolicyHelpPanelProps> {
    ...
    return (
      <Panel
        ...
        onRenderHeader={() => (
          <div className={styles.panelBanner}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, flex: 1 }}>
              <Icon iconName="Help" className={styles.bannerIcon} />
              <div>
                <h2 className={styles.bannerTitle}>Policy Help Center</h2>
                <p className={styles.bannerSubtitle}>Find answers, learn features, and get support</p>
              </div>
            </div>
            <button
        ...
        styles={{
          navigation: { height: 0, padding: 0, margin: 0, overflow: 'hidden', pointerEvents: 'none', selectors: { '.ms-Panel-closeButton': { display: 'none' } } },
          commands: { marginTop: 0, paddingTop: 0 },
          scrollableContent: { display: 'flex', flexDirection: 'column' },
          content: { padding: '0 32px 24px 32px' },
        }}
      >
        <div className={styles.helpPanelContent}>

          {/* Show article detail or main view */}
          {selectedArticle ? (

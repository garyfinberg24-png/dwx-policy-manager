  ...
  private loadQuizQuestions = async (quizId: number): Promise<void> => {
    try {
      ...
      const questions: IQuizQuestion[] = items.map((item: Record<string, unknown>) => ({
        ...
        Options: (() => { try { return item.Options ? JSON.parse(item.Options as string) : []; } catch { return []; } })(),
        CorrectAnswer: (() => { try { return item.CorrectAnswer ? JSON.parse(item.CorrectAnswer as string) : ''; } catch { return ''; } })(),

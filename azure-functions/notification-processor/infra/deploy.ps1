# ============================================================================
# DWx Policy Manager — Notification Queue Processor (Logic App) Deployment
# ============================================================================
# Deploys an Azure Logic App that:
#   1. Polls PM_NotificationQueue SharePoint list every N minutes
#   2. Sends queued notification emails via Office 365 connector
...
#   1. Go to Resource Group → dwx-pm-notif-rg-{env}
#   2. Click on 'office365-notif-{env}' connection → Edit API connection → Authorize
#   3. Click on 'sharepointonline-notif-{env}' connection → Edit API connection → Authorize
...
$ResourceGroupName = "$BaseName-notif-rg-$Environment"
$LogicAppName = "$BaseName-notification-processor-$Environment"
...
Write-Info "Batch Size:       $BatchSize notifications per run"
if ($SenderEmail) {
...
if ($WhatIf) {
    ...
    az group create `
        ...
        --location $Location `
        --tags project="DWx Policy Manager" component="Notification Processor" environment=$Environment `
        --output none
...
if ($WhatIf) {
    ...
    $deploymentOutput = az deployment group create `
        ...
        --parameters $ParametersFile `
        --parameters $overrides `
        --name "notification-processor-$Environment-$(Get-Date -Format 'yyyyMMdd-HHmmss')" `
...
Write-Host "  2. Click 'office365-notif-$Environment' > Edit API connection > Authorize" -ForegroundColor Yellow
Write-Host "     Sign in with a mailbox that has permission to send emails" -ForegroundColor Yellow
Write-Host "  3. Click 'sharepointonline-notif-$Environment' > Edit API connection > Authorize" -ForegroundColor Yellow
Write-Host "     Sign in with an account that has access to PM_NotificationQueue" -ForegroundColor Yellow
...
Write-Host ""
Write-Host "  Notification Flow:" -ForegroundColor Cyan
Write-Host "  SPFx App --> NotificationService --> PM_NotificationQueue (SP List)" -ForegroundColor Gray
Write-Host "           --> Logic App polls every $PollingMinutes min" -ForegroundColor Gray
Write-Host "           --> Office 365 sends notification email" -ForegroundColor Gray
Write-Host "           --> Status updated: Queued -> Processing -> Sent/Failed" -ForegroundColor Gray
Write-Host "           --> Failed notifications retry up to 3 times" -ForegroundColor Gray
Write-Host ""

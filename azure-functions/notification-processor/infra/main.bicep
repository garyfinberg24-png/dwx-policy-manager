// ============================================================================
// DWx Policy Manager — Notification Queue Processor (Logic App)
// ============================================================================
// Provisions: Logic App (Consumption), Office 365 + SharePoint API connections
// Reads PM_NotificationQueue SharePoint list, sends emails via Office 365, updates status.
//
// Usage:
...
@description('SharePoint site URL containing PM_NotificationQueue list')
param sharePointSiteUrl string = 'https://mf7m.sharepoint.com/sites/PolicyManager'

@description('How often (in minutes) the Logic App polls for queued notifications')
@minValue(1)
@maxValue(60)
...
@description('Max notifications to process per run')
@minValue(1)
@maxValue(50)
...
var logicAppName = '${baseName}-notification-processor-${environment}'
var office365ConnectionName = 'office365-notif-${environment}'
var sharepointConnectionName = 'sharepointonline-notif-${environment}'
...
resource office365Connection 'Microsoft.Web/connections@2016-06-01' = {
  ...
  tags: tags
  properties: {
    displayName: 'DWx PM Office 365 Notifications'
    api: {
...
resource sharepointConnection 'Microsoft.Web/connections@2016-06-01' = {
  ...
  tags: tags
  properties: {
    displayName: 'DWx PM SharePoint Online Notifications'
    api: {
...
// ============================================================================
// Logic App — Notification Queue Processor
// ============================================================================
  ...
  properties: {
    ...
    definition: {
      ...
      actions: {
        ...
        // Step 1: Query PM_NotificationQueue for Status='Queued'
        ...
        // Step 2: Process each notification sequentially
        For_Each_Queued_Notification: {
          type: 'Foreach'
          runAfter: {
            Get_Queued_Notifications: [ 'Succeeded' ]
          }
          foreach: '@body(\'Get_Queued_Notifications\')?[\'value\']'
          ...
          actions: {
            ...
            Set_Status_Processing: {
              ...
              inputs: {
                ...
                method: 'patch'
                path: '/datasets/@{encodeURIComponent(encodeURIComponent(\'${sharePointSiteUrl}\'))}/tables/@{encodeURIComponent(encodeURIComponent(\'${notificationQueueListName}\'))}/items/@{encodeURIComponent(items(\'For_Each_Queued_Notification\')?[\'ID\'])}'
                body: {
                  Status: 'Processing'
            ...
            Send_Email: {
              ...
              inputs: {
                ...
                path: '/v2/Mail'
                body: {
                  To: '@{replace(items(\'For_Each_Queued_Notification\')?[\'To\'], \';\', \',\')}'
                  Cc: '@{if(empty(items(\'For_Each_Queued_Notification\')?[\'CC\']), \'\', replace(items(\'For_Each_Queued_Notification\')?[\'CC\'], \';\', \',\'))}'
                  Subject: '@items(\'For_Each_Queued_Notification\')?[\'Subject\']'
                  Body: '@items(\'For_Each_Queued_Notification\')?[\'Body\']'
                  Importance: '@{if(equals(items(\'For_Each_Queued_Notification\')?[\'Priority\'], \'Urgent\'), \'High\', if(equals(items(\'For_Each_Queued_Notification\')?[\'Priority\'], \'High\'), \'High\', if(equals(items(\'For_Each_Queued_Notification\')?[\'Priority\'], \'Low\'), \'Low\', \'Normal\')))}'
                  IsHtml: true
            ...
            Mark_As_Sent: {
              ...
              inputs: {
                ...
                body: {
                  Status: 'Sent'
                  SentAt: '@{utcNow()}'
                  AttemptCount: '@{add(int(coalesce(items(\'For_Each_Queued_Notification\')?[\'AttemptCount\'], \'0\')), 1)}'
                }
              }
            }
            ...
            Handle_Send_Failure: {
              ...
              inputs: {
                ...
                path: '/datasets/@{encodeURIComponent(encodeURIComponent(\'${sharePointSiteUrl}\'))}/tables/@{encodeURIComponent(encodeURIComponent(\'${notificationQueueListName}\'))}/items/@{encodeURIComponent(items(\'For_Each_Queued_Notification\')?[\'ID\'])}'
                body: {
                  Status: '@{if(greaterOrEquals(add(int(coalesce(items(\'For_Each_Queued_Notification\')?[\'AttemptCount\'], \'0\')), 1), parameters(\'maxRetryAttempts\')), \'Failed\', \'Queued\')}'
                  AttemptCount: '@{add(int(coalesce(items(\'For_Each_Queued_Notification\')?[\'AttemptCount\'], \'0\')), 1)}'
                  LastAttemptAt: '@{utcNow()}'

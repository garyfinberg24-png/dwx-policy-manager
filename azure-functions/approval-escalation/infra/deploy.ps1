# ============================================================================
# DWx Policy Manager — Approval Escalation (Logic App) Deployment
# ============================================================================
# Deploys an Azure Logic App that:
#   1. Polls PM_Approvals SharePoint list every N minutes
#   2. Finds approvals where Status='Pending' AND DueDate is past
#   3. Updates overdue approvals: IsOverdue=true, Status='Escalated'
#   4. Queues urgent notifications to PM_NotificationQueue
#
# Prerequisites:
...
#   .\deploy.ps1 -PollingMinutes 10                 # Poll every 10 minutes
#
# Post-deployment:
#   After deploying, you MUST authorize the API connection in the Azure Portal:
#   1. Go to Resource Group → dwx-pm-escalation-rg-{env}
#   2. Click on 'sharepointonline-{env}' connection → Edit API connection → Authorize
#   3. Run the Logic App manually once to verify
# ============================================================================
...
$ResourceGroupName = "$BaseName-escalation-rg-$Environment"
$LogicAppName = "$BaseName-approval-escalation-$Environment"
...
if ($WhatIf) {
    ...
    az group create `
        ...
        --location $Location `
        --tags project="DWx Policy Manager" component="Approval Escalation" environment=$Environment `
        --output none
...
if ($WhatIf) {
    ...
    $deploymentOutput = az deployment group create `
        ...
        --parameters $ParametersFile `
        --parameters $overrides `
        --name "approval-escalation-$Environment-$(Get-Date -Format 'yyyyMMdd-HHmmss')" `
    ...
    Write-Info "SP Connection:   $($outputs.apiConnectionId.value)"
}
...
Write-Host ""
Write-Host "  IMPORTANT — You must authorize the API connection:" -ForegroundColor Yellow
Write-Host ""
Write-Host "  1. Go to Azure Portal > Resource Group: $ResourceGroupName" -ForegroundColor Yellow
Write-Host "  2. Click 'sharepointonline-$Environment' > Edit API connection > Authorize" -ForegroundColor Yellow
Write-Host "     Sign in with an account that has access to PM_Approvals" -ForegroundColor Yellow
Write-Host "  3. Open the Logic App > Overview > Run Trigger > Run" -ForegroundColor Yellow
Write-Host "     Verify the first run succeeds in the Run History tab" -ForegroundColor Yellow
Write-Host ""
Write-Host "  Escalation Flow:" -ForegroundColor Cyan
Write-Host "  Logic App polls every $PollingMinutes min" -ForegroundColor Gray
Write-Host "           --> Finds Pending approvals past DueDate" -ForegroundColor Gray
Write-Host "           --> Updates: IsOverdue=true, Status='Escalated'" -ForegroundColor Gray
Write-Host "           --> Queues notification to PM_NotificationQueue" -ForegroundColor Gray
Write-Host "           --> (Subject: 'Approval Overdue', Priority: 'Urgent')" -ForegroundColor Gray
Write-Host ""
